...
# 1.	Show orders where Price > 20000.
high_price <- sales_data[sales_data$Price > 20000, ]
print(high_price)
...
# 3.	Show orders where Quantity > 5.
high_qt <- sales_data[sales_data$Quantity > 5, ]
print(high_qt)
...
# 5.	Combine two conditions.
combined <- sales_data[sales_data$Price > 20000 & sales_data$City == "Pune", ]
print(combined)
...
# 1. If discount > 10 mark "High Discout
sales_data$Discount_Status <- ifelse(sales_data$Discount > 10, "High Discount", "Low Discount")
sales_data

# 2. Classify orders into High / Medium / Low value by using if else.
sales_data$Value_Status <- ifelse(sales_data$FinalAmount > 50000, "High Value",
...
while(i <= nrow(sales_data)){
  total_revenue <- total_revenue + sales_data$Revenue[i]
  i <- i + 1
}
...
for(i in 1:nrow(sales_data)){
  if(sales_data$Category[i] == "Fashion") next
  print(sales_data[i,])
} 
